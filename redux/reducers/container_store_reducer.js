...
export default (state = initialState, action) => {
    ...
    if(action.payload && 
        ...
        if(action.payload.state &&
            ...
            (action.type === types.CONTAINER_STORE_INIT || action.type === types.CONTAINER_STORE_UPDATE)
        ) {
            new_state[fieldName] = action.payload.state;
        }
        else if(typeof action.payload.key === "string" &&
            isValue(action.payload.value) && action.type === types.CONTAINER_STORE_UPDATE_FIELD
        ) {
